...
class dict(object):
    ...
    def _initialize(self):
        ...
        vocabulary= [k for k, v in self._word_count.items() if v > self.min_word_count]
...
def main_execution():
    ...
    helper = dict(training_json, min_word_count=3)
        ...
        for hidden_size in hidden_sizes:
            for dropout_percentage in dropout_percentages:
                for word_dim in word_dims:
                    ...
                    decoder = decoderRNN(hidden_size=hidden_size, output_size=output_dim, vocabulary_size=output_dim, word_dim=word_dim, dropout_percentage=dropout_percentage)
...
def testmodel(arg):
    ...
    for model_loc in file_full_path:
        ...
        df.to_csv("temp_result_v3.csv",index=False)
        l.append(df)
        
    res = pd.concat(l, axis =0)
    res.to_csv("final_result_v3.csv",index=False)
